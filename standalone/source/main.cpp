...
auto main(int /*argc*/, const char* const* /*argv*/) -> int {
    try {
        ...
        options.add_options()
            ...
            ("m,message", "Message to log", cxxopts::value<std::string>()->default_value("Hello,
        spdlog!"))
        ;
        */
        ...
        // File logger
        auto file_sink
            = std::make_shared<spdlog::sinks::basic_file_sink_mt>("digraphx_standalone.log", true);
        ...
        spdlog::debug("Sum of numbers: {}",
                      sum);  // debug level will only show in file log by default
