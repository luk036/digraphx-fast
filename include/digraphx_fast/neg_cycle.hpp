    ...
    template <typename Graph> class NegCycleFinder {
        ...
        auto _find_cycle() -> std::optional<node_t> {
            ...
            for (node_t v = 0; v < V; ++v) {
                if (_visited[v] != node_t(-1)) continue;
                auto u = v;
                _visited[u] = v;
                while (_pred_node[u] != node_t(-1)) {
                    u = _pred_node[u];
                    if (_visited[u] == v) return u;
                    if (_visited[u] != node_t(-1)) break;
                    _visited[u] = v;
                }
            }
            return {};
        ...
        auto _is_negative(node_t handle, const std::vector<weight_t>& weights,
            ...
            do {
                auto u = _pred_node[v];
                auto e = _pred_edge[v];
                if (dist[v] > dist[u] + weights[e]) return true;
                v = u;
            } while (v != handle);
            return false;
        }
      ...
      public:
        explicit NegCycleFinder(const Graph& graph)
            : _graph(graph),
              _pred_node(graph.num_nodes, node_t(-1)),
              _pred_edge(graph.num_nodes, 0),
              _visited(graph.num_nodes, 0) {}
         ...
         */
        template <typename Fn> auto howard(std::vector<weight_t>& dist,
                                           const std::vector<weight_t>& weights, Fn&& yield_cycle,
                                           size_t max_cycles = 1) -> bool {
            // Reset predecessor map
            std::fill(_pred_node.begin(), _pred_node.end(), node_t(-1));

            size_t found = 0;
            while (found < max_cycles) {
                if (!_relax(dist, weights)) break;
         ...
         */
        template <typename Fn> auto howard_warm(std::vector<weight_t>& dist,
